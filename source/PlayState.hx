...
class PlayState extends MusicBeatState
{
		...
		if (!PlayStateChangeables.Optimize)
		{
			...
			switch(stageCheck)
			{
				...
				case 'street-rain':
				{
					...
					InStageAnimation("assets/trollge/images/background/test.webm");
				}
				case 'void':
				{
					defaultCamZoom = 0.7;
	...
	function startCountdown():Void
	{
		...
		switch(storyDifficulty)
		{
			...
			case 1: healthFactor = healthFactor * 0.5;
	...
	override public function update(elapsed:Float)
	{
		...
		scoreTxt.x = (originalX - (lengthInPx / 2)) + 275;
				...
				if ((daNote.mustPress && daNote.tooLate && !PlayStateChangeables.useDownscroll || daNote.mustPress && daNote.tooLate && PlayStateChangeables.useDownscroll) && daNote.mustPress)
				{
					...
					else
					{
						if (loadRep && daNote.isSustainNote)
						{
							...
							if (findByTime(daNote.strumTime) != null) totalNotesHit += 1;
							else
							{
								vocals.volume = 0;
								if (theFunne)
	...
	public function InStageAnimation(source:String)
	{
		#if cpp
	...
	function goodNoteHit(note:Note, resetMashViolation = true):Void
	{
		...
		if (!note.wasGoodHit)
		{
			if (!note.isSustainNote)
			{
				...
				popUpScore(note);
				combo += 1;
				note.arriveTime = timer;
			}
			else
			{
				totalNotesHit += 1;
				if (health < 2) health += 0.015;
			}
